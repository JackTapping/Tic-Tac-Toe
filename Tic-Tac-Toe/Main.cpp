...
//print pices to the visual board 
...
//takes tha input from the player and updates that position in the GameStateArray
...
int main()
{
	...
	bool bKeepPlaying = true; //becomes false when a game is won/draw 
	bool bFreeSpace = false; // becomes true if the program finds a '.' in the GameSateArray
	bool bVailidInput = false; //beomse true if the user enters in a number or the letter e
	bool bGameIsADraw = false; // becomes true if the programe cant find a '.' in the GameSateArray

	char GameStateArray[3][3] = { '.','.','.',    '.','.','.',    '.','.','.', }; // '.' represents an empty space on the board 
	...
	while (cInput != 'e' &&  cInput != 'E')
	{
		switch (cInput)
		{
		case '1': // this what loads if the PVP menu option is picked 
			InitializeGame();
			...
			system("cls");
			break;
		case '2': // this is what loads if the CPU menu option is picked 
			InitializeGame();
		...
		WipeBoard(GameStateArray); //resets the elements to '.' for the GameSateArray
		bKeepPlaying = true;
		bFreeSpace = false;
		bVailidInput = false;
		iPlayerWon = -1;
		...
		//re loading the menu for the user to pick a new game or close the program 
		PrintMenu();
		cInput = GetMenuInput();
		FlashSelectedOption(cInput);
		std::cout << "\n\n";
	...
	//if the user has selected e in the menu game will close and goodbye mesage will play befor the program closes 
	system("cls");
	PrintThankYouScreen();
	PatchyFade();
	system("cls");
